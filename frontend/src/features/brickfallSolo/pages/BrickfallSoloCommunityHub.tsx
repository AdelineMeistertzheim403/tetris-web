...
import { BrickfallSoloCommunityLevelCard } from "../components/BrickfallSoloCommunityLevelCard";
import "../../../styles/pixel-protocol-hub.css";

export default function BrickfallSoloCommunityHub() {
  const PAGE_SIZE = 12;
  ...
  useEffect(() => {
    ...
    void fetchBrickfallSoloCommunityLevels()
      .then((levels) => {
  ...
  const handleToggleLike = async (level: BrickfallSoloCommunityLevel) => {
    if (!user || level.isOwn) return;
    try {
      const result = await toggleBrickfallSoloCommunityLevelLike(level.id);
  ...
  const filteredLevels = useMemo(() => {
    ...
    const next = communityLevels.filter((level) => {
      ...
      return `${level.level.name} ${level.authorPseudo}`.toLowerCase().includes(query);
    });
    ...
    next.sort((a, b) => {
      ...
      return b.likeCount - a.likeCount || new Date(b.updatedAt ?? 0).getTime() - new Date(a.updatedAt ?? 0).getTime();
    });
    return next;
  ...
  return (
    <div className="pp-hub font-['Press_Start_2P']">
      <div className="pp-hub-shell">
        <h1 className="pp-hub-title">Brickfall Solo Joueurs</h1>
          ...
          {pagedLevels.length > 0 && (
            <div className="pp-hub-community-list">
              {pagedLevels.map((level) => (
                <BrickfallSoloCommunityLevelCard
                  key={level.id}
                  level={level}
                  canLike={Boolean(user)}
                  onPlay={(item) => navigate(`/brickfall-solo/play?community=${encodeURIComponent(item.id)}`)}
